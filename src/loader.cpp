...
namespace {
template <typename T>
T& readAtOffset(std::span<uint8_t> f, ptrdiff_t offset) noexcept {
  return *reinterpret_cast<T*>(&f[offset]);
}

std::string_view readAtOffset(std::span<uint8_t> const f, ptrdiff_t offset) noexcept {
  return { reinterpret_cast<char const*>(&f[offset]) };
}

template <typename T>
std::span<T> readManyAtOffset(std::span<uint8_t> f, ptrdiff_t offset, size_t amount, size_t size) noexcept {
...
std::optional<std::pair<SharedObject, LoadPhase>> findSharedObject(std::filesystem::path const& dependencyDir,
  ...
  for (auto it = loadPhaseMap.arr.crbegin(); it != loadPhaseMap.arr.crend(); it++) {
    if (it->first != phase) {
      continue;
    }
...
SharedObject::getToLoad(std::filesystem::path const& dependencyDir, LoadPhase phase,
  ...
  auto getSectionHeader = [&](size_t s) {
    return *reinterpret_cast<Elf64_Shdr*>(reinterpret_cast<uint8_t*>(f.data()) + elf.e_shoff + (elf.e_shentsize * s));
  };
  ...
  for (size_t s = 0; s < elf.e_shnum; s++) {
    ...
    for (size_t d = 0; d < num_deps; d++) {
      auto const& dyn = *reinterpret_cast<Elf64_Dyn*>(reinterpret_cast<uint8_t*>(f.data()) + sectionHeader.sh_offset +
                                                      (sectionHeader.sh_entsize * d));
...
std::vector<SharedObject> listAllObjectsInPhase(std::filesystem::path const& dependencyDir, LoadPhase phase) {
  for (auto const& [ph, path] : loadPhaseMap.arr) {
    if (ph == phase) {
      ...
      for (auto const& file : std::filesystem::directory_iterator(dependencyDir / path)) {
...
std::vector<LoadResult> loadMod(SharedObject&& mod, std::filesystem::path const& dependencyDir,
  ...
  if (skipLoad.contains(mod.path)) {
    // TODO: Log
    LOG_WARN("Already loaded mod at path: %s", mod.path.c_str());
    return {};
  }
  ...
  auto handleResult = [&](OpenLibraryResult&& result, SharedObject&& obj,
    ...
    if (auto const* error = get_if<std::string>(&result)) {
      return FailedMod(std::move(obj), std::move(*error), std::move(dependencies));
    }
